...
| 4 | **텍스트 줄바꿈** | reference PNG가 Pretendard 없이 캡처되어 배지·칩·버튼이 2줄로 깨져 있음. 실제 구현은 1줄 | **제외** — 원본 CSS를 따름 |
...
| 10 | **API 히어로 우측 빈 컬럼** | 원본이 `1fr 340px` 그리드인데 우측 셀이 비어 있음. 좌측 폭을 결정하므로 **그대로 재현** | 재현 대상 (차이 아님) |
...
**주의:** reference PNG들은 Pretendard가 로드되지 않은 상태로 캡처된 것으로 보인다.
배지/칩/버튼 텍스트가 원본 CSS상으로는 줄바꿈되지 않아야 하는데 2줄로 깨져 있다
(예: 01-home의 "AI-Ready 공간 데이터 플랫폼" 배지, 07-api의 "문서 보기" 버튼,
08-request-landing의 "Custom Data Request" 배지, 칩 다수).
**텍스트 줄바꿈 차이는 reference 캡처 아티팩트로 간주하고 원본 CSS를 따른다.**
레이아웃·간격·색상·타이포 크기는 reference를 기준으로 검증한다.
