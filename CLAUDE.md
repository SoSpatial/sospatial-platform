...
**reference 캡처 환경에 대한 검증 결과 (2026-08-09, 네비 검증 시점)**

당초 "reference는 Pretendard 없이 캡처된 것 같다"고 추정했으나 **이 추정은 틀렸다.**
구현 페이지를 (A) Pretendard 로드 / (B) CDN 차단 두 조건으로 촬영해
reference와 각각 픽셀 차분한 결과:
...
차단 시 차이가 오히려 2.9배 늘었다. **reference는 Pretendard로 렌더된 것이 맞다.**
따라서 폰트 관련 차이를 "캡처 아티팩트"로 넘기지 말 것.

남은 1.44%는 **글자 획 굵기(래스터라이즈) 차이**로, reference 쪽 글자가 미세하게 더
...
**배지·버튼 줄바꿈 현상 — 원인 미확정, 수정 금지**

01-home의 "AI-Ready 공간 데이터 플랫폼" 배지, 07-api의 "개발자용" 배지와
"문서 보기" 버튼, 08-request-landing의 "Custom Data Request" 배지가
reference 에서 2줄로 줄바꿈돼 있다. 원본 CSS 로는 줄바꿈될 이유가 없다.

**홈 히어로 검증에서 얻은 관찰 (2026-08-09):**
reference 배지의 **박스 폭이 구현과 동일(≈199px)한데 텍스트만 2줄**이다.
박스가 min-content 로 잘못 계산된 것이었다면 박스 자체가 좁아졌어야 하는데
그렇지 않다. 즉 **박스 크기 계산은 정상이고 그 안에서 텍스트만 넘쳤다.**
같은 폭의 박스에서 내 렌더는 딱 맞고 reference 는 한 음절이 밀려났으므로,
reference 쪽 텍스트가 미세하게 더 넓게 조판된 것으로 보인다.
이는 앞서 확인한 "reference 글자 획이 미세하게 더 두껍다"와 방향이 일치한다.

⚠ 이 관찰은 **"reference 가 Pretendard 없이 캡처됐다"는 가설을 뒷받침하지 않는다.**
그 가설은 위 표의 통제 실험으로 이미 반증됐다(폰트 차단 시 차이 2.9배 악화).
동일 폰트를 쓰되 래스터라이저/조판이 다른 환경에서 캡처된 것으로 본다.

**조치: 수정하지 않는다.** reference 에 맞추려면 원본 CSS 에 없는 폭 제약을
임의로 만들어야 한다. 이후 페이지에서 같은 현상이 나와도 보고만 하고 넘어갈 것.
