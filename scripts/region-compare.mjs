...
const [, , refFile, implFile, y0 = '0', y1 = '700', out = 'region.png'] = process.argv
const Y0 = Number(y0)
const Y1 = Number(y1)
const H = Y1 - Y0
const W = 1440
...
const win = (url, label) => `
  <div class="label">${label} &nbsp;<span class="dim">y ${Y0}–${Y1}</span></div>
  <div class="win" style="width:${W}px;height:${H}px">
    <div class="img" style="background-image:url(${url});background-size:${W}px auto;background-position:0 -${Y0}px"></div>
  </div>`
